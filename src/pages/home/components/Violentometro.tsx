...
const ThermometerApp = () => {
    ...
    const topicList = [
        ['trata', 'Trata virtual de personas'],
        ['difusion', 'Difusión de contenido íntimo sin consentimiento'],
        ['sextorsion', 'Sextorsión'],
        ['difamacion', 'Difamación virtual'],
        ['suplantacion', 'Suplantación virtual'],
        ['hostigamiento', 'Hostigamiento virtual'],
        ['acecho', 'Acecho virtual'],
        ['insultos', 'Insultos electrónicos'],
    ]
    ...
    const [temperatureHeight, setTemperatureHeight] = useState<string>('0%');
    const [topic, setTopic] = useState<Description>(termoDescriptions.insultos)
    const [topicColor, setTopicColor] = useState<string>(termoColors.insultos)
    ...
    const handleClick = (topic: string) => {
        setTemperature(termoValues[topic as keyof typeof termoValues]);
        setTopic(termoDescriptions[topic as keyof typeof termoDescriptions]);
        setTopicColor(termoColors[topic as keyof typeof termoColors]);
        switch (topic) {
        ...
        <Wrapper>
            ...
            <div className="options">
                {topicList.map((topic, i) => {
                    return (<div key={i} onClick={() => handleClick(topic[0])}>
                        <p style={selected === i + 1 ? { fontWeight: 'bold' } : {}}>
                            {topic[1]}
                        </p>
                    </div>)
                })}
